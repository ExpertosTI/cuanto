...
async function sendEmailSmtp({ to, subject, html }) {
  ...
  // Lightweight SMTP via nodemailer if present; else fail with clear error
  let nodemailer
  try {
...
function otpEmailHtml(code) {
  ...
  <div style="font-family:Arial,sans-serif;max-width:480px;margin:0 auto;padding:20px">
    ...
    <p style="color:#5f7268;font-size:14px">Código de verificación (master)</p>
...
async function sendWhatsAppCloud({ toPhone, text }) {
  ...
  if (!res.ok) {
    ...
    throw new Error(`whatsapp_${res.status}:${body.slice(0, 120)}`)
  }
}
